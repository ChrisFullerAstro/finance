...
def classfication():
    ...
    logging.info('current_transaction:{0}'.json.dumps(session['current_transaction'] ))
    ...
    logging.info('current_transaction after classfication:{0}'.json.dumps(current_transaction))
    ...
    if automatic:
        ...
        logging.info('current_transaction after auto ready for injection into master:{0}'.json.dumps(fct))
        db_finance.db.master.insert_one(fct)

        logging.info('current_transaction after auto ready for injection into current_transactions:{0}'.json.dumps(ct))
        if session.get('current_transactions'):
